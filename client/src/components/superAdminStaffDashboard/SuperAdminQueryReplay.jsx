...
import { fetchQueryById } from "../../redux/superAdminStaff/superAdminStaffReplayIdSlice";
import { postStaffReplyMessage } from "../../redux/superAdminStaff/superAdminStaffReplayIdPostSlice";
...
const SuperAdminStaffQueryReply = ({ id, goBack }) => {
  const dispatch = useDispatch();
  const chatRef = useRef(null);
  const [message, setMessage] = useState("");

  const { selectedQuery, loading, error, successMessage } = useSelector(
    (state) => state.superAdminStaffSendQueryIdReplay
  );
  useEffect(() => {
    if (id) {
      dispatch(fetchQueryById(id));
    }
    // 🔹 Call fetchSuperAdminQueries and log the result
  }, [dispatch, id]);
  ...
  const handleSend = () => {
    if (message.trim()) {
      dispatch(postStaffReplyMessage({ id, message })).then(() => {
        setMessage("");
        dispatch(fetchQueryById(id));
      });
    }
...
export default SuperAdminStaffQueryReply;
