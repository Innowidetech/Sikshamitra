...
import { createMeeting, clearMessages } from '../../redux/parent/meetingSlice';
import { fetchTeacherNames } from '../../redux/parent/querySlice';
...
const ScheduleMeeting = () => {
  ...
  const { teacherNames, loading: teacherLoading } = useSelector((s) => s.query);
  ...
  const handleGenerateLink = () => {
    const attendants = [
      ...(sendToAdmin ? ['Admin'] : []),
      ...selectedTeachers.map((name) => name.split('-')[0].trim()),
    ];
  ...
  return (
    <div className="max-w-4xl mx-auto border border-blue-300 rounded-md p-6 mt-10">
      <div className="mb-4">
        <button onClick={() => navigate(-1)} className="text-blue-600 hover:underline text-sm flex items-center gap-1">
          ← Back
        </button>
      </div>
